...
*/
    ...
    * @param  mixed  $Value The value on the right side of the expression. If $EscapeValueSql is true then it will end up in a parameter. <b>Syntax</b> The $Field and Value expressions can begin with special characters to do certain things. <ul> <li><b>=</b>: This means that the argument is a function call. If you want to pass field reference arguments into the function then enclose them in square brackets. ex. <code>'=LEFT([u.Name], 4)'</code> will call the LEFT database function on the u.Name column.</li> <li><b>@</b>: This means that the argument is a literal. This is useful for passing in literal numbers.</li> <li><b>no prefix></b>: This will treat the argument differently depending on the argument. - <b>$Field</b> - The argument is a column reference. - <b>$Value</b> - The argument will become a named parameter. </li></ul>
    *
    * <b>Syntax</b>
    ...
    * @param string $Data  An associative array of FieldName => Value pairs that should be inserted $Table.
    * $Table.
    */
    ...
    * $Table.
    * @param mixed $Where  A where clause (or array containing multiple where clauses) to be applied to the where portion of the update statement.
    ...
    * @param  boolean $EscapeSql    A boolean value indicating if $this->EscapeSql method should be called on $Field.
    * on $Field.
    ...
    * @param mixed  $Set    An associative array (or object) of FieldName => Value pairs that should be inserted, or an array of FieldName values that should have values inserted from $Select.
    ...
    * @param array  $Where The columns to find the row to update. If a row is not found then one is inserted and the items in this array are merged with $Set.
    ...
    * @param  string $On        The conditions on which the join should occur. ie. "user.role_id = role.id"
    * ie. "user.role_id = role.id"
    * @param  string $Join      The type of join to be made. Accepted values are: 'inner', 'outer', 'left', 'right', 'left outer', and 'right outer'.
    ...
    *
    * @param  mixed  $Field  The field name (or array of field name => match values) to search in for a like $Match. a like $Match.
    * a like $Match.
    ...
    * clause.
    * @param  string $Side   A string indicating which side of the match to place asterisk operators. Accepted values are left, right, both, none. Default is both.
 ...
 * A convenience method for Gdn_DatabaseDriver::Like that changes the operator to 'not like,'
    ...
    * @param mixed  $Set   An array of $FieldName => $Value pairs, or an object of $DataSet->Field properties containing one rowset.
    ...
    *
    * @param  mixed  $Select   NotRequired "*" The field(s) being selected. It can be a comma delimited string, the name of a single field, or an array of field names. can be a comma delimited string, the name of a single field, or an array of field names.
    ...
    * @param  array  $Options The options and results in an associative array. A blank key will be the final "else" option of the case statement. eg. array('null' => 1, '' => 0) results in "when null then 1 else 0".
    ...
    *
    * @param  mixed   $Field                   The name of the field to save value as. Alternately this can be an array of $FieldName => $Value pairs, or even an object of $DataSet->Field properties containing one rowset. of $FieldName => $Value pairs, or even an object of $DataSet->Field properties containing one rowset.
    ...
    *
    * @param  mixed   $Field             The string on the left side of the comparison, or an associative array of Field => Value items to compare. Field => Value items to compare.
    * Field => Value items to compare.
    * @param  mixed   $Value             The string on the right side of the comparison. You can optionally provide an array of DatabaseFunction => Value, which will be converted to DatabaseFunction('Value'). If DatabaseFunction contains a '%s' then sprintf will be used for to place DatabaseFunction into the value. provide an array of DatabaseFunction => Value, which will be converted to DatabaseFunction('Value'). If DatabaseFunction contains a '%s' then sprintf will be used for to place DatabaseFunction into the value.
    ...
    * @param  boolean $EscapeFieldSql    A boolean value indicating if $this->EscapeSql method should be called on $Field.
    * on $Field.
