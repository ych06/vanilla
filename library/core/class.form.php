...
class Gdn_Form extends Gdn_Pluggable
{
    ...
    * @param array  $Attributes An associative array of attributes for the button. Here is a list of "special" attributes and their default values: Attribute  Options                        Default ------------------------------------------------------------------------ Type       The type of submit button      'submit' Value      Ignored for $ButtonCode        $ButtonCode translated
    ...
    *
    * @param  string $FieldName  The name of the field that is being displayed/posted with this input. It should related directly to a field name in $this->_DataArray.
    ...
    */
    ...
    {
        include_once PATH_LIBRARY.'/vendors/recaptcha/functions.recaptchalib.php';
    ...
    * @param array $Options   An associative array of options for the select. Here is a list of "special" options and their default values: Attribute     Options                        Default ------------------------------------------------------------------------ Value         The ID of the category that    FALSE is selected. IncludeNull   Include a blank row?           TRUE CategoryData  Custom set of categories to    CategoryModel::Categories() display.
    ...
    *
    * @param  string $FieldName  Name of the field that is being displayed/posted with this input. It should related directly to a field name in $this->_DataArray.
    *    It should related directly to a field name in $this->_DataArray.
    * @param  string $Label      Label to place next to the checkbox.
    * @param  array  $Attributes Associative array of attributes for the input. (e.g. onclick, class)\ Setting 'InlineErrors' to FALSE prevents error message even if $this->InlineErrors is enabled.
    ...
    *
    * @param mixed  $DataSet      Data to fill the checkbox list. Either an associative array or a database dataset. ex: RoleID, Name from GDN_Role. array or a database dataset. ex: RoleID, Name from GDN_Role.
    * array or a database dataset. ex: RoleID, Name from GDN_Role.
    *
    * @param mixed  $ValueDataSet Values to be pre-checked in $DataSet. Either an associative array or a database dataset. ex: RoleID from GDN_UserRole for a single user.
    * or a database dataset. ex: RoleID from GDN_UserRole for a single user.
    *
    * @param array  $Attributes   An associative array of attributes for the select. Here is a list of "special" attributes and their default values: Attribute   Options                        Default ------------------------------------------------------------------------ ValueField  The name of the field in       'value' $DataSet that contains the option values. TextField   The name of the field in       'text' $DataSet that contains the option text. "special" attributes and their default values: Attribute   Options                        Default ------------------------------------------------------------------------ ValueField  The name of the field in       'value' $DataSet that contains the option values. TextField   The name of the field in       'text' $DataSet that contains the option text.
    * "special" attributes and their default values:
    * Attribute   Options                        Default
    * ------------------------------------------------------------------------
    ...
    *
    * @param string $FieldName    The name of the field that is being displayed/posted with this input. It should related directly to a field name in a user junction table. ie. LUM_UserRole.RoleID
    * should related directly to a field name in a user junction table.
    * ie. LUM_UserRole.RoleID
    *
    * @param mixed  $DataSet      The data to fill the options in the select list. Either an associative array or a database dataset. ie. RoleID, Name from LUM_Role. array or a database dataset. ie. RoleID, Name from LUM_Role.
    * array or a database dataset. ie. RoleID, Name from LUM_Role.
    *
    * @param mixed  $ValueDataSet The data that should be checked in $DataSet. Either an associative array or a database dataset. ie. RoleID from LUM_UserRole for a single user.
    ...
    * @param  string $FieldName  The name of the field that is being displayed/posted with this input. It should related directly to a field name in $this->_DataArray.
    *    should related directly to a field name in $this->_DataArray.
    * @param  array  $Attributes An associative array of attributes for the input, e.g. onclick, class. Special attributes: YearRange, specified in yyyy-yyyy format. Default is 1900 to current year. Fields, array of month, day, year. Those are only valid values. Order matters.
    *    Special attributes:
    ...
    *
    * @param string $FieldName  The name of the field that is being displayed/posted with this input. It should related directly to a field name in $this->_DataArray. ie. RoleID
    *    should related directly to a field name in $this->_DataArray. ie. RoleID
    * @param mixed  $DataSet    The data to fill the options in the select list. Either an associative array or a database dataset. array or a database dataset.
    ...
    *
    * @param array  $Group     An array of $PermissionName => $CheckBoxXhtml to be rendered within the grid. This represents the final (third) part of the permission name string, as in the "Edit" part of "Garden.Roles.Edit". ie. 'Edit' => '<input type="checkbox" id="PermissionID" name="Role/PermissionID[]" value="20" />'; grid. This represents the final (third) part of the permission name string, as in the "Edit" part of "Garden.Roles.Edit". ie. 'Edit' => '<input type="checkbox" id="PermissionID" name="Role/PermissionID[]" value="20" />';
    ...
    *
    * @param array  $Rows      An array of rows to appear in the grid. This represents the middle part of the permission name, as in the "Roles" part of "Garden.Roles.Edit". of the permission name, as in the "Roles" part of "Garden.Roles.Edit".
    * of the permission name, as in the "Roles" part of "Garden.Roles.Edit".
    *
    * @param array  $Cols      An array of columns to appear in the grid for each row. This (again) represents the final part of the permission name, as in the "Edit" part of "Garden.Roles.Edit". ie. Row1 = array('Add', 'Edit', 'Delete'); represents the final part of the permission name, as in the "Edit" part of "Garden.Roles.Edit". ie. Row1 = array('Add', 'Edit', 'Delete');
    ...
    *
    * @param  string $FieldName  The name of the field that is being hidden/posted with this input. It should related directly to a field name in $this->_DataArray.
    * should related directly to a field name in $this->_DataArray.
    * @param  array  $Attributes An associative array of attributes for the input. ie. maxlength, onclick, class, etc
    * class, etc
    * @return string
    */
    ...
    * @param  array  $Attributes An associative array of attributes for the input. (e.g. maxlength, onclick, class) Setting 'InlineErrors' to FALSE prevents error message even if $this->InlineErrors is enabled.
    *    Setting 'InlineErrors' to FALSE prevents error message even if $this->InlineErrors is enabled.
    * @return string
    */
...
PASSWORDMETER;
    ...
    * @param array  $Attributes      Associative array of attributes for the input that the label is for. This is only available in case the related input has a custom id specified in the attributes array. This is only available in case the related input has a custom id specified in the attributes array.
    ...
    */
    public function Open($Attributes = array()) 
    {
        ...
        $Return = '<form';
        if ($this->InputPrefix != '' || array_key_exists('id', $Attributes)) { $Return .= $this->_IDAttribute(
            $this->InputPrefix,
            $Attributes
        ); 
        }
        ...
        if ($this->Method != "get") {
            ...
            if ($HoneypotName) { $Return .= $this->Hidden(
                $HoneypotName,
    ...
    * @param  array  $Attributes Associative array of attributes for the input (e.g. onclick, class). Special values 'Value' and 'Default' (see RadioList).
    ...
    * @param string $Value      The value being assigned in the hidden input. Unless $ForceValue is changed to TRUE, this field will be retrieved from the form upon postback.
    ...
    *
    * @param Gdn_Model $Model   The Model that will enforce data rules on $this->_DataArray. This value is passed by reference so any changes made to the model outside this object apply when it is referenced here.
    ...
    * @param type  $Options Additional options to pass into the form. - Wrap: A two item array specifying the text to wrap the form in. - ItemWrap: A two item array specifying the text to wrap each form item in.
    ...
    * @param  array  $Attributes An associative array of attributes for the input. ie. maxlength, onclick, class, etc. If $Attributes contains an 'id' key, it will override the one automatically generated by $FieldName.
    ...
    * @param  array  $Attributes An associative array of attributes for the input. ie. maxlength, onclick, class, etc. If $Attributes contains a 'name' key, it will override the one automatically generated by $FieldName.
    ...
    * @param  array  $Attributes An associative array of attributes for the input. ie. maxlength, onclick, class, etc. If $Attributes contains a 'value' key, it will override the one automatically generated by $FieldName.
