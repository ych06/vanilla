...
class Gdn_Configuration extends Gdn_Pluggable
{
    ...
    *
    * @param  string $Name         The name of the configuration setting to get. If the setting is contained within an associative array, use dot denomination to get the setting. ie. <code>$this->Get('Database.Host')</code> would retrieve <code>$Configuration[$Group]['Database']['Host']</code>.
    ...
    *
    * @param string  $Name      The name of the configuration setting to assign. If the setting is contained within an associative array, use dot denomination to get the setting. ie. $this->Set('Database.Host', $Value) would set $Configuration[$Group]['Database']['Host'] = $Value contained within an associative array, use dot denomination to get the setting. ie. $this->Set('Database.Host', $Value) would set $Configuration[$Group]['Database']['Host'] = $Value
    ...
    * @param  string  $Name    The name of the variable and initial group settings. Note: When $Name is 'Configuration' then the data will be set to the root of the config. Note: When $Name is 'Configuration' then the data will be set to the root of the config.
    ...
    * @param  string  $Name       The name of the variable and initial group settings. Note: When $Name is 'Configuration' then the data will be set to the root of the config. Note: When $Name is 'Configuration' then the data will be set to the root of the config.
    *   Note: When $Name is 'Configuration' then the data will be set to the root of the config.
    * @param  boolean $Dynamic    Optional, whether to treat this as the request's "dynamic" config, and to save config changes here. These settings will also be re-applied later when "OverlayDynamic" is called after all defaults are loaded.
    *   to save config changes here. These settings will also be re-applied later when "OverlayDynamic"
    *   is called after all defaults are loaded.
    * @return boolean
    */
    ...
    * @param bool|array   $Options Bool = whether to save or not. Assoc array = Save => Whether to save or not
    ...
    */
    public static function FromFile($Parent, $File, $Name = 'Configuration') 
    {
        ...
        if ($Parent && $Parent->Caching()) {
            ...
            if (Gdn::Cache()->Type() == Gdn_Cache::CACHE_TYPE_MEMORY && Gdn::Cache()->ActiveEnabled()) {
                $UseCache = true;
                $CachedConfigData = Gdn::Cache()->Get(
                    $FileKey, array(
                    Gdn_Cache::FEATURE_NOPREFIX => true
                    )
                );
    ...
    */
    ...
    public function Save() 
    {
        ...
        if ($this->Callback && is_callable($this->Callback)) {
            ...
            $CallbackOptions = array_merge(
                $CallbackOptions, $this->CallbackOptions, array(
        ...
        case 'file':
            ...
            $FileContents = Gdn_Configuration::Format(
                $Data, array(
                'VariableName'    => $Group,
                'WrapPHP'         => true,
                'ByLine'          => true
            ...
            if ($this->Configuration && $this->Configuration->Caching() && Gdn::Cache()->Type() == Gdn_Cache::CACHE_TYPE_MEMORY && Gdn::Cache()->ActiveEnabled()) {
                $CachedConfigData = Gdn::Cache()->Store(
                    $FileKey, $Data, array(
                    Gdn_Cache::FEATURE_NOPREFIX  => true,
                    Gdn_Cache::FEATURE_EXPIRY    => 3600
                    )
