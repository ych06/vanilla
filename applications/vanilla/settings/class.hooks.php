...
class VanillaHooks implements Gdn_IPlugin
{
    ...
    */
    public function SiteNavModule_default_handler($sender) 
    {
        ...
        if (Gdn::Session()->IsValid()) {
            $sender->addLink(
                'favorites.bookmarks', array('text' => t('My Bookmarks'),
            ...
            $sender->addLink(
                'favorites.discussions', array('text' => t('My Discussions'),
                'url' => '/discussions/mine', 'icon' => icon('discussion'),
                'badge' => countString(Gdn::Session()->User->CountDiscussions))
            );
            $sender->addLink(
                'favorites.drafts', array('text' => t('Drafts'), 'url' => '/drafts',
                'icon' => icon('compose'),
    ...
    */
    public function SiteNavModule_profile_handler($sender) 
    {
        ...
        $sender->addLink(
            'posts.discussions', array('text' => t('Discussions'), 'url' => UserUrl($user, '', 'discussions'),
            'icon' => icon('discussion'), 'badge' => countString(val('CountDiscussions', $user), "/profile/count/discussions?userid=$user_id"))
        );

        $sender->addLink(
            'posts.comments', array('text' => t('Comments'), 'url' => UserUrl($user, '', 'comments'),
