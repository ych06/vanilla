...
class CategoryModel extends Gdn_Model
{
    ...
    */
    protected static function rebuildLock($release = false) 
    {
        ...
        if (is_null($isMaster)) {
            ...
            $masterKey = Gdn::cache()->add(
                self::MASTER_VOTE_KEY, $instanceKey, array(
    ...
    * @param  bool|int $adjustDepth    Whether or not to adjust the depth or a number to adjust the depth by. Passing `true` as this parameter will make the returned subtree look like the full tree which is useful for many views that expect the full category tree.
